 ...
 */
public class MegatronGraphitePluginTest {
  ...
  public void test() throws Exception {
    ...
    try (UdpServer udpServer = new UdpServer(Integer.parseInt(configuration.getProperty("megatron.graphite.port")))) {
      try (MegatronTestApi api = new MegatronTestApi(); AbstractMegatronUdpPlugin plugin = new MegatronGraphitePlugin()) {
        ...
        assertThat(udpServer.getReceivedTest().replaceAll(" \\d{9,11}", ""))
            .isEqualTo(contentOf(getClass().getResource("/out.txt"), StandardCharsets.UTF_8).replaceAll(" \\d{9,11}", ""));
      }
    }
  }
